...
#    %% Piecewise constant segmented signal
    ...
    if Nd==0:
      ...
      if Nd==1:
        mc=m[krmv[0]]*np.ones(N) #% m(k)*ones(1,k-krmv(1))]
      else:
        mc=np.zeros(k)
        mc[0:int(krmv[0])]=m[int(krmv[0])]
        cc=krmv[0]+1
        for ii in range(1,Nd):
            cc2=cc+(krmv[ii]-krmv[ii-1])
            mc[int(cc):int(cc2)]= m[int(krmv[ii])] 
            cc=cc2+1
        mc[int(cc):]= m[int(k)]
...
def tvdip(y, lambdas, display=1, stoptol=1e-3, maxiter=60,z=None):
    ...
    if display:
        sys.stdout.write("lambda_max=%5.2e\n" % lambdamax)
...
#    for idx, l in enumerate(lambdas):
    ...
    for iters in xrange(maxiter):
        ...
        gap = pobj - dobj
        if display:
            sys.stdout.write( "{:5d} {:7.2e} {:7.2e} {:7.2e}\n".format(iters, pobj, dobj,gap))
    ...
    if display:
        if s:
            sys.stdout.write(("Solved to precision of duality gap %5.2e\n") % gap)
        else:
            sys.stdout.write(("Max iterations exceeded - solution may be inaccurate\n"))
...
def _OverlapAddW(X,YPhase,windowLen,ShiftLen,wnd):
    ...
    weight=np.zeros(int( (FrameNum-1)*ShiftLen+windowLen ))
...
def _segment(signal,W,SP,wnd):
    ...
    Seg= np.zeros((int(W),int( N) ),dtype=np.complex64)
    cc=0
