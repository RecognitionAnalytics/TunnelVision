function output=WienerScalartBA(signal,fs,IS)
...
W=fix(.05*fs); %Window length is 25 ms
...
for i=1:numberOfFrames
    ...
    if SpeechFlag==0 % If not Speech Update Noise Parameters
        N=(NoiseLength*N+N0 + Y(:,i))/(NoiseLength+2); %Update and smooth noise mean
        LambdaD=(NoiseLength*LambdaD+LambdaD0+(Y(:,i).^2))./(2+NoiseLength); %Update and smooth noise variance
    end
