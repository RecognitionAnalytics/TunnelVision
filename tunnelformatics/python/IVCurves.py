...
class IVCurve(Trace):
    ...
    def calcStatistics(self):
        if self.isControl==False:
            controlTrace=self._experiment.getNearestIVControl(self.isIonic)
        else:
            controlTrace=None
        
        if self==controlTrace:
            controlTrace=None
...
#        plt.plot(V,I,Volts,Current)
#        plt.xlabel('Volts (' + str(V.units) + ')')
#        plt.ylabel('Current (' + str(I.units) + ')')
#        plt.title('IV Curve')
#        plt.grid(True)
#        plt.show()
#        
...
class tunnelIV(IVCurve):
    ...
    def _simmonsFormula2(self,p,V):#dang guy (simmons) changed units every other line.  This is just to get everything kosher.  It would make sense to optimize this to nice units before trying to use it in a curve fit
          ...
          p2=Vn*(s2+s1)/(2*s)
          p2.units=pq.eV
          phiI=phi0-p2-(1.15*L*s/(s2-s1))*np.log(s2*(s-s1)/(s1*(s-s2)))
...
#        myoutput.pprint()
        ...
        plt.plot(Volts/1000,Current_Corrected,Volts/1000,simI)
        plt.xlabel('Volts (' + str(Volts.units) + ')')
        plt.ylabel('Current (' + str(Current_Corrected.units) + ')')
        plt.title('Tunnel fit' + self.stats.fileName)
        plt.grid(True)
        plt.show()
    ...
    def __init__(self,fullPath,experiment):
        IVCurve.__init__(self,fullPath,experiment)
...
#        CapT*1e9,IVparam.Est_Solution_Resistance*1e-9,(1/Conductance)*1e-9
        ...
        plt.xlabel('Volts (' + str(Volts.units) + ')')
        plt.ylabel('Current (' + str(Current_Corrected.units) + ')')
        plt.title('Ionic Fitted Curve' + self.stats.fileName)
        plt.grid(True)
        plt.show()
